...
class WeightsConstrainedMLP(nn.Module):
    ...
    def __init__(
        ...
        if monotonic_indices is None:
            monotonic_indices = list(range(input_size))
        if len(monotonic_indices) != len(set(monotonic_indices)):
            raise ValueError("monotonic_indices contains duplicates")
        if any(i < 0 or i >= input_size for i in monotonic_indices):
            raise ValueError("monotonic_indices contains an out-of-range index")
        self.monotonic_indices = list(monotonic_indices)
        first_layer_mask = torch.zeros(input_size, dtype=torch.bool)
        first_layer_mask[self.monotonic_indices] = True
        self.register_buffer("first_layer_monotonic_mask", first_layer_mask)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.layers):
            # Only constrained input columns must be non-negative in the first
            # layer. Later layers remain entirely non-negative so those paths
            # cannot reverse the constrained inputs' effects.
            positive_weights = transform_weights(layer.weight, self.transform)
            if i == 0:
                mask = self.first_layer_monotonic_mask.unsqueeze(0)
                effective_weights = torch.where(mask, positive_weights, layer.weight)
            else:
                effective_weights = positive_weights
            x = F.linear(x, effective_weights, layer.bias)
