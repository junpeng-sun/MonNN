...
def cross_validate(
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        train_ds = TensorDataset(torch.FloatTensor(X_train[fit_idx]), torch.FloatTensor(y_train[fit_idx]).reshape(-1, 1))
        stop_ds = TensorDataset(torch.FloatTensor(X_train[stop_idx]), torch.FloatTensor(y_train[stop_idx]).reshape(-1, 1))
        ...
        train_loader = DataLoader(train_ds, batch_size=int(config["batch_size"]), shuffle=True, generator=g)
        stop_loader = DataLoader(stop_ds, batch_size=int(config["batch_size"]), shuffle=False)
        val_loader = DataLoader(val_ds, batch_size=int(config["batch_size"]), shuffle=False)
...
def main():
    ...
    results_file = experiment_result_file(__file__, "main", "exps_SMNN.csv")
