...
class MinMaxNetwork(nn.Module):
    def __init__(
        ...
        init_method: Literal['xavier_uniform', 'xavier_normal', 'kaiming_uniform', 'kaiming_normal',
            'he_uniform', 'he_normal', 'truncated_normal'] = 'truncated_normal',
        ...
        self.h_K = h_K
        self.device = device
        monotonic_mask = torch.zeros(input_size, dtype=torch.bool, device=self.device)
        monotonic_mask[monotonic_indices] = True
        self.register_buffer("monotonic_mask", monotonic_mask)
        ...
        # Match the reference min-max implementation: affine weights and
        # offsets are both initialized from a truncated normal distribution.
        for weight, bias in zip(self.z, self.t):
            init_weights(weight, method=init_method)
            init_weights(bias, method=init_method)
    ...
    def init_weights(self, method: Literal['xavier_uniform', 'xavier_normal', 'kaiming_uniform', 'kaiming_normal', 'he_uniform', 'he_normal', 'truncated_normal']) -> None:
        ...
        for weight, bias in zip(self.z, self.t):
            init_weights(weight, method=method)
            init_weights(bias, method=method)
...
class MinMaxNetworkWithMLP(nn.Module):
    def __init__(
        ...
        init_method: Literal['xavier_uniform', 'xavier_normal', 'kaiming_uniform', 'kaiming_normal', 'he_uniform', 'he_normal', 'truncated_normal'] = 'truncated_normal',
        transform: Literal['exp', 'explin', 'sqr'] = 'exp',
        use_sigmoid: bool = False,
        aux_activation: Optional[Callable[[torch.Tensor], torch.Tensor]] = None
    ):
        """
...
class SmoothMinMaxNetwork(nn.Module):
    def __init__(
        ...
        beta: float = -1.0,
        init_method: Literal['xavier_uniform', 'xavier_normal', 'kaiming_uniform', 'kaiming_normal', 'he_uniform', 'he_normal', 'truncated_normal'] = 'truncated_normal',
        transform: Literal['exp', 'explin', 'sqr'] = 'sqr',
        use_sigmoid: bool = False
    ):
        """
        ...
        # Match the official SMM initialization: both affine weights and
        # biases use a unit-scale truncated normal; beta keeps beta_init.
        for weight, bias in zip(self.z, self.t):
            init_weights(weight, method=init_method)
            init_weights(bias, method=init_method)
